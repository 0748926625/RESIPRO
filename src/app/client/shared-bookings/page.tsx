...
export default async function ClientSharedBookingsPage({
  searchParams,
}: {
  ...
  const { data } = await supabase
    ...
    .select("id, requested_start, requested_end, status, properties(name, city, slug)")
    ...
    <div className="mx-auto flex w-full max-w-2xl flex-1 flex-col gap-6 px-4 py-8">
      ...
      {requests.length === 0 ? (
        <p className="text-sm text-foreground/60">
          Aucune demande de partage.{" "}
          <Link href="/residences" className="underline">
        ...
        <ul className="flex flex-col divide-y divide-foreground/10 rounded-md border border-foreground/10">
          {requests.map((request) => (
            <li key={request.id} className="flex items-center justify-between gap-4 px-4 py-3 text-sm">
              <div>
                <p className="font-medium text-foreground">{request.properties?.name ?? "Résidence"}</p>
              ...
              {request.status === "searching_partner" ? (
                <form action={cancelSharedBookingRequest.bind(null, request.id, "/client/shared-bookings")}>
